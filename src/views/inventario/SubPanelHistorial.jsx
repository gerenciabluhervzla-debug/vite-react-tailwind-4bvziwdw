...
import { Calendar, Save, FileDown, Search, Loader2, CheckCircle, SlidersHorizontal } from 'lucide-react';
...
export default function SubPanelHistorial({ lista, movimientos, pedidos = [], db, appId, loggear, dialogs, puedeEditar }) {
  ...
  // Re-calcula la tabla de hoy en caso de que quieran guardar
  const metricasHoy = useMemo(() => {
    ...
    pedidos.forEach(p => {
       if (p.status !== 'Anulado' && p.status !== 'Rechazado' && (p.fechaDespacho === hoyDDMM || new Date(p.fechaCreacion).toISOString().split('T')[0] === fechaConsulta)) {
  ...
  const guardarCierreHoy = async () => {
    dialogs.confirm("¿Deseas guardar la foto del inventario de HOY? Esto sobreescribirá el cierre anterior de esta misma fecha.", async () => {
       setGuardando(true);
       try {
          const filas = lista.map(item => {
             ...
             return {
                ...
                ventas: m.ventas,
                cierreEnvios: item.envios,
                cierreRecep: item.recepcion
             };
          });

          await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'cierres_inventario', hoyISO), { filas, fechaGuardado: Date.now() }, { merge: true });
          loggear('CIERRE_INVENTARIO', `Guardó snapshot del inventario del día ${hoyISO}`);
          setCierreActivo(filas);
          dialogs.alert("Cierre del día guardado exitosamente. Ahora estará disponible en el historial.", "Éxito");
       } catch(e) {
          dialogs.alert("Error guardando el cierre.");
       }
       setGuardando(false);
    });
  };
  ...
  const filasFiltradas = soloCambios ? dataMostrar.filter(f => f.ingresos > 0 || f.traslados > 0 || f.ventas > 0) : dataMostrar;

  return (
    <div className="animate-in fade-in space-y-6 print:m-0 print:p-0">
      
      {/* HEADER CONTROLES - OCULTO AL IMPRIMIR */}
      <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-6 justify-between items-center print:hidden">
         ...
         <div className="flex flex-wrap gap-3 w-full md:w-auto">
            {cierreActivo ? (
               ...
               fechaConsulta === hoyISO && puedeEditar && (
                  <button onClick={guardarCierreHoy} disabled={guardando} className="w-full md:w-auto bg-amber-500 hover:bg-amber-600 text-white px-6 py-3 rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg transition-transform hover:-translate-y-0.5 disabled:opacity-50">
                     {guardando ? <Loader2 className="animate-spin" size={18}/> : <Save size={18}/>} Generar y Guardar Cierre de Hoy
                  </button>
               )
            )}
         </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden print:border-none print:shadow-none print:bg-white print:text-black">
         ...
         {!cierreActivo && !cargando ? (
            <div className="p-16 text-center flex flex-col items-center justify-center print:hidden">
               ...
               {fechaConsulta === hoyISO && puedeEditar && <p className="text-sm mt-2 text-slate-400">Utiliza el botón superior para capturar el inventario actual y cerrar el día.</p>}
            </div>
         ) : (
            <div className="overflow-x-auto print:overflow-visible">
               <table className="w-full text-left text-sm border-collapse min-w-[800px] print:min-w-full">
                  <thead>
                     <tr className="bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 text-[10px] uppercase tracking-wider print:bg-gray-100 print:text-black">
                        ...
                        <th className="p-4 border-b border-slate-200 dark:border-slate-700 print:border-gray-300 text-center font-black bg-sky-50 dark:bg-sky-900/10 print:bg-transparent">Cierre ENVÍOS</th>
                  ...
                  <tbody className="print:text-xs">
                     {filasFiltradas.map((f, i) => (
                        <tr key={i} className="border-b border-slate-100 dark:border-slate-700/50 print:border-gray-200">
                           ...
                           <td className="p-3 text-center font-black bg-sky-50/30 dark:bg-sky-900/5 print:bg-transparent print:text-black">{f.cierreEnvios}</td>
